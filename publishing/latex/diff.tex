...
%DIF DEL first-submission/paper.tex   Thu Aug 13 14:04:36 2015
%DIF ADD revision/revision.tex        Fri Aug 14 15:17:56 2015
...
\section*{Abstract}
\DIFdelbegin \DIFdel{Recently }\DIFdelend 3D printing has emerged as a method for directly printing complete microfluidic devices, although printing materials have been limited to oxygen-impermeable materials.
...
This is the first 3D-printed device \DIFdelbegin \DIFdel{that can be functionalized to control oxygen }\DIFdelend \DIFaddbegin \DIFadd{incorporating gas permeable membranes to facilitate oxygen control }\DIFaddend in cell culture.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \linenumbers
...
\DIFaddbegin \DIFadd{In this study, a larger 24 well version was developed and optimized which includes several key improvements over the previous 6-well version.}\DIFaddend 
...
These \DIFaddbegin \DIFadd{6-well }\DIFaddend devices were completely cast from PDMS and fabricated in a multiple bonding procedure.
...
The pillars extend into each well leaving a $\sim$500 $\mu$m gap \DIFdelbegin \DIFdel{for media }\DIFdelend between the diffusion membrane and the culture surface\DIFdelbegin \DIFdel{at the bottom of the well}\DIFdelend \DIFaddbegin \DIFadd{.
This gap allows space for $\sim$0.17 mL of media}\DIFaddend .
...
\DIFdelbegin \DIFdel{The intensity of the sensor was measured with via fluorescence microscopy and correlated to the concentration of oxygen with }\DIFdelend \DIFaddbegin \DIFadd{Five calibration points were fitted to a two-site model\mbox{%DIFAUXCMD
\cite{Carraway1991,Wang2014}
}%DIFAUXCMD
of the }\DIFaddend Stern-Volmer \DIFdelbegin \DIFdel{analysis}\DIFdelend \DIFaddbegin \DIFadd{relationship which was used to correlate sensor intensities with oxygen concentration}\DIFaddend .
...
\DIFaddbegin \DIFadd{The assembled device was sterilized by spraying with 70\% ethanol solution and placed in a biosaftey hood with UV lights overnight.
}\DIFaddend Human lung adenocarcinoma (A549) cells were seeded at 30,000 cells per well in a 24-well plate.
...
\DIFaddbegin \DIFadd{No contamination of the culture was observed.
}\DIFaddend Cells in each well were washed twice with PBS, and 300 $\mu$L of a lysis solution consisting of 10 $\mu$L of $\beta$-mercaptoethanol (Sigma Aldrich) per 1 mL of lysis buffer (PureLink RNA Mini Kit; Invitrogen) was added to each well.
...
A different gas condition can be used in each separate iteration of the 6-well unit allowing 6 technical replicates of up to 4 different gas conditions in one 24-well plate\DIFdelbegin \DIFdel{(Fig.
~\ref{fig2}). }\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend There was little variation in the oxygen levels measured in each of the 6 wells controlled by a single inlet, demonstrating the distribution network worked effectively.

\begin{figure}[h]
\caption{
{\bf Oxygen Characterization.} 
\DIFdelbeginFL \DIFdelFL{(A) }\DIFdelendFL Time course data of \DIFaddbeginFL \DIFaddFL{four }\DIFaddendFL oxygen \DIFdelbeginFL \DIFdelFL{being evacuated from the culture area as 0\% oxygen gas is perfused through the device.
Error bars are the standard deviation N=3.
(B) Four oxygen }\DIFdelendFL conditions are demonstrated in a 24-well plate.
Each 6-well row of the plate can be controlled independently.  
\DIFdelbeginFL \DIFdelFL{Mean }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Error bars are the standard deviation }\DIFaddendFL N=\DIFdelbeginFL \DIFdelFL{3 error bars not included.
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.
}\DIFaddendFL }
\label{fig2}
\end{figure}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The 6 h time point was chosen based on the HIF-1$\alpha$ timecourse in A549 cells. 
HIF-1$\alpha$ protein levels are maximal at 4 h and steeply decrease by 16 h\mbox{%DIFAUXCMD
\cite{Li2006}
}%DIFAUXCMD
. 
